...
feature_columns = [
    col for col in reference_data.columns if col not in ["target", "prediction"]
]
...
try:
    ...
    report_result = report.run(
        current_data=eval_data_current, reference_data=eval_data_reference
    )
    ...
    drift_detected = (
        ...
        if hasattr(drift_results, "drift_detected")
        else False
    )
    drift_score = (
        getattr(drift_results, "drift_score", 0.0)
        if hasattr(drift_results, "drift_score")
        else 0.0
    )
