...
if model_selection == "linear":
    ...
    with mlflow.start_run(run_name="linear-regression-corn-yield"):
        ...
        mlflow.log_artifact(
            vectorizer_path, artifact_path="model"
        )  # Save in same folder
    ...
    with mlflow.start_run(run_name="Ridge-regression-corn-yield"):
        ...
        space_ridge = {"alpha": hp.loguniform("alpha", np.log(1e-4), np.log(1e3))}
    ...
    with mlflow.start_run(run_name="lasso-regression-corn-yield"):
        ...
        # Optimization for parameter alpha
        space_lasso = {"alpha": hp.loguniform("alpha", np.log(1e-4), np.log(1e3))}
        ...
        model_lasso = Pipeline(
            [
                ...
                (
                    "regressor",
                    Ridge(alpha=best_lasso["alpha"], max_iter=5000, random_state=42),
                ),
            ]
        )
    ...
    with mlflow.start_run(run_name="Gradient-Boosting-regression-corn-yield"):
        ...
        # Log the model to MLflow
        mlflow.sklearn.log_model(
            sk_model=model_gbr, artifact_path="model", registered_model_name=model_name
        )
