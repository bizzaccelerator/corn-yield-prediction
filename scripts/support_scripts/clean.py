...
def load_and_understand_data(filepath):
    ...
    corn.info()
    print(
        "The number of null values in the dataset is confirmed as:", corn.isna().sum()
    )
...
def encode_and_split_data(corn_cleaned, test_size=0.2, val_size=0.25, random_state=42):
    ...
    print(
        f"The Dataset splitted as follows: {int((1-test_size)*100)} percent for training, "
    ...
    print(
        f"The number of registries in the train dataset is {len(X_train)}, "
...
def save_outputs(
    X_train, X_val, X_test, y_train, y_val, y_test, dv, output_dir="data_splits"
):
    """
